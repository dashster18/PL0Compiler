...
int statement() {
    ...
    switch (token) {
        ...
        case callsym:
             ...
             getNextToken();
             emit(5,symbolTable[fetchIDIndex(&token)].level,symbolTable[fetchIDIndex(&token)].addr);
             getNextToken();
            break;
        ...
        case insym:
            ...
            getNextToken();
            emit(sioIn,0,2);
            emit(sto,symbolTable[fetchIDIndex(&token)].level,symbolTable[fetchIDIndex(&token)].addr);
            break;
        case outsym:
            getNextToken();
            if(token != identsym) error(1);
