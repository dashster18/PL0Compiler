...
int main(int argc, char **argv){
	...
	int verboseParser = 0;
	int verboseVM = 0;
	char* fname = "nestedProcedures.pl0";

    //fname = "/Users/Nikeshsrivastava/Dropbox/COP_3402_Project/Assignment_4/factorial.pl0";
    ...
    temp = runScanner(fn1, verboseScanner);
    ...
    if (verboseVM)
        printf("\nVirtual Machine:");
    runPM0(objectFile, fo2, verboseVM);
...
int insertConst(char* ID, int val) {
    ...
    do {
        if(symbolTable[hashval].kind == 0)
        {
            ...
            scopeList[scopeTail].kind = 1;
            scopeList[scopeTail].name = ID;
            scopeList[scopeTail].val = val;
            scopeTail++;
            return 0;
        }
...
int insertVar(char* ID) {
    ...
    do {
        if(symbolTable[hashval].kind == 0)
        {
            ...
            scopeList[scopeTail].kind = 2;
            scopeList[scopeTail].name = ID;
            scopeList[scopeTail].level = g_level;
            scopeList[scopeTail].addr = g_addr++;
            scopeTail++;
            return 0;
        }
        else if(!strcmp(ID, symbolTable[hashval].name))
...
int fetchIDIndex(char* ID)
{
    ...
    {
        //printf("ERRRG %d ",hashval);
        if(strcmp(LL->symbols[(int)(*ID)],symbolTable[hashval].name) == 0 && symbolTable[hashval].level <= g_level) {
            //printf("Found 1!");
            lastseen = hashval;
        }
        hashval = (hashval+1)%x;
...
void cleanScope()
{
    while(scopeTail >= 0 && scopeList[scopeTail].level > g_level)
    {
        scopeList[scopeTail].kind = 0;
        symbolTable[fetchIDIndex(scopeList[scopeTail].name)].kind = 0;
        scopeTail--;
    }
...
/*
 ...
 GET(TOKEN);
 if TOKEN != â€œidentsymâ€� then ERROR;
 GET(TOKEN);
 if TOKEN != "semicolomsym" then ERROR;
 GET(TOKEN);
 BLOCK;
...
int block() {
    ...
    if (token == procsym) {
        while(token == procsym)
        {
            ...
            getNextToken();
            insertProcedure(getID(token));
            int foo = fetchIDIndex(&token); // Use later to store the correct address of the function
            ...
            getNextToken();
            g_level++;
            // TODO: need to modify the address of the procedure to start before the block but after the JMP 0,0
            symbolTable[foo].addr = cx;
            errorCode = block();
    ...
    if (!(token == identsym || token == callsym || token == beginsym || token == ifsym || token == whilesym || token == insym || token == outsym)) {
        error(6);
        return 6;
    }
    errorCode = statement();
...
int statement() {
    ...
    switch (token) {
        case identsym:
            ...
            if (symbolTable[temp].kind == 1 || symbolTable[temp].kind == 3) {
                error(12);
                return 12;
            }
            ...
            if (errorCode) return errorCode;    // Propogate error up
            emit(sto, g_level - symbolTable[temp].level, symbolTable[temp].addr);
            break;
        ...
        case callsym:
            ...
            getNextToken();
            if (symbolTable[fetchIDIndex(&token)].kind != 3) {
                error(15);
                return 15;
            }
            emit(cal, g_level - symbolTable[fetchIDIndex(&token)].level, symbolTable[fetchIDIndex(&token)].addr);
            getNextToken();
            break;
        ...
        case ifsym: //
            ...
            getNextToken();
			//emit(jpc,0,cx+2); //We implemented this wrong. We have to assume 0 is true.
			start = cx;
			emit(jpc, 0, 0);
            errorCode = statement();
            if (errorCode) return errorCode;    // Propogate error up
...
int factor() {
    ...
    switch (token) {
        case identsym:
            ...
            {
				emit(lod, g_level - symbolTable[temp].level,symbolTable[temp].addr);
            }
...
void runPM0(FILE* fn1, FILE* fo2, int verboseOutput) {
...
LexemeList *runScanner(FILE *fin, int verboseOutput) {
    ...
    while (i < numChars) {
        ...
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) { // If c is a letter
            ...
            // Special Symbols: â€˜+â€™, â€˜-â€˜, â€˜*â€™, â€˜/â€™, â€˜(â€˜, â€˜)â€™, â€˜=â€™, â€™,â€™ , â€˜.â€™, â€˜ <â€™, â€˜>â€™,  â€˜;â€™ , â€™:â€™
            switch (c) {
                case '+':
                case '-':
                case '*':
    ...
    for (i = 0; i < numLexemes; i++) {
        ...
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
            ...
            else if (!strcmp(lexemes[i], "write")) {
                tokens[i] = outsym;
            }
            else if (!strcmp(lexemes[i], "read")) {
                tokens[i] = insym;
            }
            else {
