...
void printStack(int stack[], regFile registers);
...
int main(int argc, char **argv){
    ...
    if(argc < 2) {
        fn1 = fopen("/Users/Nikeshsrivastava/Dropbox/COP_3402_Project/Assignment_1/P1.in","rb");
        fo2 = fopen("/Users/Nikeshsrivastava/Dropbox/COP_3402_Project/Assignment_1/P1.out","wb");
    }
...
void runPM0(FILE* fn1, FILE* fo2) {
    ...
    for(i = 0; i < instructionCount; i++) {
        ...
        printf(" %3d %3s %3d %3d", registers.PC-1, OPCODES[code[i].OP], code[i].L, code[i].M);
        // Execute:
        execute(fo2, code, stack, &registers);
        printf(" %3d %3d %3d  \n", registers.PC, registers.BP, registers.SP);
        for(j = registers.SP; j >= 0; j--) {
            printf(" %d ", stack[j]);
        }
        printf("\n");
        ...
        if( mBP <= 1) {
            buffer = (char)(
                            i--;
                                ...
                                while(i >=1 && i >= mBP) {
                                    sprintf(buffer, "%d %s", stack[i], buffer);
                                    i--;
                                }
                                sprintf(buffer, "l %s", buffer);
                                mBP = mDL;
                                mDL = mBP + 2;
                            }
                            }
